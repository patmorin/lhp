...
class tripod_partition(object):
    ...
    def h8parents(self, t, i):
        ...
        for p in set(self.h3parents(t)):
            pp = max(p, 1)
            if self.tripod_tree[pp][1] and self.tripod_tree[pp][1] > t:
                j = 0
            elif self.tripod_tree[pp][2] and self.tripod_tree[pp][2] > t:
                j = 1
            else:
                j = 2
    ...
    def verify_results(self):
        ...
        # Make sure the graph obtained by contracting tripods has treewidth 3
        for t in range(len(self.tripods)):
            h3parents = self.h3parents(t)
            assert(len(h3parents) <= 3)
            assert(len(h3parents) == 0 or max(h3parents) < t)

        # self.tripod_colours = self.colour_tripods()
